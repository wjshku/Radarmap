axiosFunc = async() => {    //this function update the data
  ...
  for(let i = 0;i<response.data.total_items;i++){
    ...
    for(let j = 0;j< num_dim;j++){
      if(response.data.items[i].answers[j].type==="choice"){
      data[i].push(response.data.items[i].answers[j].choice.label)
      console.log(data)
      }else{
        data[i].push(response.data.items[i].answers[j].number)
      }
    }
  }
  console.log(data)
  ...
  function dataclear(data){
    ...
    let cleardata = []
    let existing = {}
    cleardata.push(new city(realdata[0].answers[0].choice))
